 ...
 * |[
 * gst-launch-1.0 v4l2src ! videoconvert ! cvdlibobjectdetecthog model-location=model.dat ! videoconvert ! xvimagesink
 * ]|
 * </refsect2>
 */
...
G_DEFINE_TYPE_WITH_PRIVATE (GstCVDlibObjectDetectHOG,
    gst_cv_dlib_object_detect_hog, GST_TYPE_CV_OBJECT_DETECT);

static GstFlowReturn gst_cv_dlib_object_detect_hog_transform_ip (
    GstOpencvVideoFilter *base, GstBuffer *buf, cv::Mat img);
void gst_cv_dlib_object_detect_hog_detect (GstCVObjectDetect *self,
    cv::Mat &img, GstCVObjectDetectContext *ctx);
...
static void gst_cv_dlib_object_detect_hog_set_property (GObject *object, guint
    prop_id, const GValue *value, GParamSpec *pspec);
static void gst_cv_dlib_object_detect_hog_get_property (GObject *object, guint
...
gst_cv_dlib_object_detect_hog_finalize (GObject *obj)
{
  ...
  GstCVDlibObjectDetectHOGPrivate *priv =
      (GstCVDlibObjectDetectHOGPrivate *)
      gst_cv_dlib_object_detect_hog_get_instance_private (
      self);

  g_free (self->model_location);
  delete priv->detector;
...
gst_cv_dlib_object_detect_hog_class_init (GstCVDlibObjectDetectHOGClass *klass)
{
  ...
  gobject_class->finalize = GST_DEBUG_FUNCPTR (
      gst_cv_dlib_object_detect_hog_finalize);
  ...
  g_object_class_install_property (gobject_class, PROP_MODEL_LOCATION,
      ...
      "The location of the serialized model to classify against.", NULL,
      (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

  gst_element_class_set_static_metadata (element_class, "cvdlibobjectdetecthog",
      "Filter/Effect/Video", "Detect objects using a HOG-based detector",
...
read_model_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
  ...
  gbarray = g_file_read_to_byte_array_finish (G_FILE (source_object), res,
      &error);

  if (error != NULL) {
    GST_ELEMENT_ERROR (self, RESOURCE, READ,
        ("Error while reading from file \"%s\".", self->model_location), ("%s",
          error->message));
    return;
  }
  ...
  try {
    ...
    const char *error_info = e.info.c_str ();
    GST_WARNING_OBJECT (self, "Cannot deserialize model at '%s': %s",
        self->model_location, error_info);
  }
}

static void
gst_cv_dlib_object_detect_hog_load_model (GstCVDlibObjectDetectHOG *self, const
    gchar *location)
{
  GFile *file;
  char buffer[4096];
  ...
  g_file_read_to_byte_array_async (file, buffer, sizeof (buffer), NULL,
      read_model_cb, self);
}
