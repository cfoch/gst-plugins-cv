    ...
    guint index, GstCVObjectInfo * info)
{
  ...
  original_list =
      cv_object_info_map_get_object_info_list_at_index (self, index);
...
static void
_list_foreach (GstCVObjectInfo * object_info,
    GstCVObjectInfoMapFuncData * func_data)
{
  func_data->foreach_func (func_data->index, object_info, func_data->user_data);
}

static void
_table_foreach (guint index, GList * object_info_list,
    GstCVObjectInfoMapFuncData * func_data)
{
...
gst_cv_object_info_map_foreach (GstCVObjectInfoMap * self,
    GstCVObjectInfoMapFunc func, gpointer user_data)
{
